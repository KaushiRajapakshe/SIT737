...
3. Create a new branch named `sit737-2025-prac6p`. [Git Branch](https://github.com/KaushiRajapakshe/SIT737/tree/sit737-2025-prac4c)
4. Clone the repository to your local machine:
   ```bash
   git clone https://github.com/KaushiRajapakshe/SIT737.git
5. Swith to the branch in your local machine:
    ```bash
    git checkout sit737-2025-prac6p
6. Navigate to the folder `sit737-2025-prac6p` and Run `npm install` command.
7. Build the docker container run `docker build -t calculator .` command.
8. Push container to docker repository `docker login` and `docker push kaushi11/calculator:latest`.
9. Deploy using kubernet `kubectl apply -f createDeployment.yaml` and `kubectl apply -f createService.yaml`.
10. Application is listening on `3041` port. http://localhost:3041.
11. To remove the kubernet service and deployment run `kubectl delete -f createService.yaml` and `kubectl delete -f createDeployment.yaml`.
